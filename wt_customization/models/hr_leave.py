    ...
    def _check_holidays(self):
        for holiday in self:
            ...
            if holiday.holiday_type != 'employee'\
                ...
                continue
            if not holiday.holiday_status_id.allows_negative:
                if holiday.employee_id:
                    if holiday.holiday_status_id.allows_negative:
                        leave_days = mapped_days[holiday.employee_id.id][holiday.holiday_status_id.id]
                        if float_compare(leave_days['remaining_leaves'], 0, precision_digits=2) > -holiday.holiday_status_id.max_allowed_negative\
                            or float_compare(leave_days['virtual_remaining_leaves'], 0, precision_digits=2) == -holiday.holiday_status_id.max_allowed_negative:
