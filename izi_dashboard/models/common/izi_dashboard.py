...
class IziDashboardSlide(models.Model):
    ...
    def default_get(self, fields):
        res = super(IziDashboardSlide, self).default_get(fields)
        if 'default_dashboard_id' in self.env.context:
            res['dashboard_id'] = self.env.context['default_dashboard_id']
        return res
    ...
    # handle old odoo 15 version. 
    @api.model
    def create(self, vals):
        context = self.env.context
        if not vals['dashboard_id']:
            vals['dashboard_id'] = context.get('default_dashboard_id')
        res = super(IziDashboardSlide, self).create(vals)
        return res
    ###############################
