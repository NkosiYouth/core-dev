...
class CustomerPortalHelpdesk(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "ticket_count" in counters:
            ...
            domain = []
            done_stage_ids = request.env["helpdesk.ticket.stage"].search([('closed','=',True)])
            domain += [('stage_id','not in', done_stage_ids.ids)]
            employee_id = request.env['hr.employee'].sudo().search([]).filtered(lambda x:x.related_contact_ids.ids == request.env.user.partner_id.ids)
            if employee_id and employee_id.user_roles_id.name == "Youth":
                domain += [('partner_youth_id.id','=',employee_id.id)]
            elif employee_id and employee_id.user_roles_id.name == "Supervisor":
                employee_ids = request.env['hr.employee'].sudo().search([('parent_id.id','=',employee_id.id)])
                employee_id_list = []
                employee_id_list.append(employee_id.id)
                for rec in employee_ids:

                    employee_id_list.append(rec.id)

                domain += [('partner_youth_id.id','in', employee_id_list)]
            ticket_count = (
                helpdesk_model.sudo().search_count(domain)
    ...
    def portal_my_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in=None, groupby=None, **kw ):  
        ...
        for team in request.env["helpdesk.ticket.team"].sudo().search([]):
            searchbar_filters[str(team.id)] = {
                "label": team.name,
        ...
        done_stage_ids = request.env["helpdesk.ticket.stage"].sudo().search([('closed','=',True)])
        domain += [('stage_id','not in', done_stage_ids.ids)]
        tickets = HelpdeskTicket.sudo().search(
                domain,
                order=order,
    ...
    def _ticket_get_searchbar_groupby(self):
        values = {
            ...
            "team": {"input": "team", "label": _("Team"), "order": 2},
        }
